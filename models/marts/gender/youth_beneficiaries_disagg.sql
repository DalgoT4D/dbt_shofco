...
subsequent_data AS (
    SELECT
        ...
        coalesce(
            s.date_of_registration, i.date_of_registration
        ) AS date_of_registration,
        coalesce(
            s.beneficiary_gender, i.beneficiary_gender
        ) AS beneficiary_gender,
        coalesce(
            s.name_of_the_beneficiary, i.name_of_the_beneficiary
        ) AS name_of_the_beneficiary,
        coalesce(
            s.beneficiary_phone_number, i.beneficiary_phone_number
        ) AS beneficiary_phone_number,
        coalesce(s.registered_by, i.registered_by) AS registered_by,
        coalesce(
            s.beneficiary_categories, i.beneficiary_categories
        ) AS beneficiary_categories,
        ...
        unnest(
            string_to_array(
                coalesce(s.beneficiary_categories, i.beneficiary_categories),
                ' '
            )
        ) AS category
