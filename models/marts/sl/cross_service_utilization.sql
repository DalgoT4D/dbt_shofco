...
with base as (
    select
        ...
        (
            (income_on_average_pl is not null and trim(income_on_average_pl) != '')
            or (placement_opportunity_pl is not null and trim(placement_opportunity_pl) != '')
        ) as participated_job_placements,

        -- total service count
        (case when digital_literacy_dl is not null or start_date_dl is not null or advanced_it_start_date_dl is not null or completion_date_dl is not null then 1 else 0 end +
         ...
         case when swep_tailoring_center_st is not null or start_date_of_training_st is not null then 1 else 0 end +
         ...
         case when (income_on_average_pl is not null and trim(income_on_average_pl) != '') or (placement_opportunity_pl is not null and trim(placement_opportunity_pl) != '') then 1 else 0 end) as total_services_count,

        -- service combination
        concat_ws(' + ',
            case when digital_literacy_dl is not null or start_date_dl is not null or advanced_it_start_date_dl is not null or completion_date_dl is not null then 'Digital Literacy' end,
            ...
            case when swep_tailoring_center_st is not null or start_date_of_training_st is not null then 'Tailoring' end,
...
service_grain as (
    ...
    from base
    cross join lateral (
        select 'Digital Literacy' as service, participated_digital_literacy as participated
        union all
        select 'Mentorship', participated_mentorship
        union all
        select 'Apprenticeship', participated_apprenticeship
        union all
        select 'Internship', participated_internship
        union all
        select 'Handcraft', participated_handcraft
        union all
        select 'IGA', participated_iga
        union all
        select 'Tailoring', participated_tailoring
        union all
        select 'TVET', participated_tvet
        union all
        select 'Business Grants', participated_business_grants
        union all
        select 'Entrepreneurship', participated_entrepreneurship
        union all
        select 'Coworking', participated_coworking
        union all
        select 'Job Placements', participated_job_placements
    ) as services
