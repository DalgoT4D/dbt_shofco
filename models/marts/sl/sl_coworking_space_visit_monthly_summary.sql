...
aggregated AS (
    ...
    FROM source
    GROUP BY year_of_visit, month_of_visit, county_shofco_site, subcounty_shofco_site, ward_shofco_site
)

SELECT 
    year_of_visit,
    month_of_visit,
    CONCAT(year_of_visit, '-', LPAD(month_of_visit::text, 2, '0')) AS month,
    county_shofco_site,
    subcounty_shofco_site,
    ward_shofco_site,
    total_visits,
    ...
    other_visits
FROM aggregated
ORDER BY year_of_visit DESC, month_of_visit DESC
