...
SELECT
    ...
    CASE
        WHEN data -> 'form' ->> 'date' ~ '^\d{4}-\d{2}-\d{2}$'
            THEN
                CASE
                    WHEN
                        EXTRACT(
                            MONTH FROM TO_DATE(
                                data -> 'form' ->> 'date', 'YYYY-MM-DD'
                            )
                        ) BETWEEN 1 AND 3
                        THEN 'Term 1'
                    WHEN
    ...
    -> 'form'
    -> 'school_population'
    ->> 'number_of_health_club_members_boys' AS health_club_boys,
    data
    -> 'form'
    -> 'school_population'
    ->> 'number_of_health_club_members_girls' AS health_club_girls,
    ...
    CASE
        WHEN
            data
            -> 'form'
            -> 'monitoring_indicator_2'
            ->> 'an_active_and_representative_health_club_established_check_the_record_of_th'
            = 'yes'
            THEN 1
        ELSE 0
    ...
    (
        CASE
            WHEN
                data
                -> 'form'
                -> 'monitoring_indicator_1'
                ->> 'weekly_health_club_meetings_conducted_check_the_club_register_and_minutes_o'
                = 'yes'
                THEN 1
            ELSE 0
        END
