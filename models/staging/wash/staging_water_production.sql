...
WITH lindi_cte AS (
    ...
    WHERE
        ...
        AND "BH_METER" IS NOT NULL
        AND CAST("TREATED_Production" AS NUMERIC) >= 0
),
...
subra_cte AS (
    SELECT
        ...
        CAST(
            (
                CAST("B2_Abstraction" AS NUMERIC)
                + CAST("BH1_Abstraction" AS NUMERIC)
            ) AS VARCHAR
        ) AS bh_production,
        "TREATED_Production" AS treated_consumption,
        'Subra' AS tank
    ...
    WHERE
        CAST("B2_Abstraction" AS NUMERIC) + CAST("BH1_Abstraction" AS NUMERIC)
        >= 0
        AND "BH_1_METER" IS NOT NULL
        AND CAST("TREATED_Production" AS NUMERIC) >= 0
)
